    ...
    <div className="h-[calc(100vh-8rem)] overflow-x-auto">
       <div className="flex h-full space-x-4 min-w-[1200px] pb-4">
         {columns.map(col => {
           ...
           return (
             <div key={col.id} className={`flex-1 min-w-[280px] rounded-xl flex flex-col border ${col.color}`}>
               <div className="p-3 border-b border-inherit bg-white/50 rounded-t-xl backdrop-blur-sm">
                 ...
                 {colInstalls.map(inst => {
                   ...
                   >
                     <div className="flex justify-between items-start mb-2">
                       <span className="text-xs font-bold text-slate-400">#{inst.id}</span>
                       <span className="text-xs font-bold bg-slate-100 text-slate-600 px-2 py-0.5 rounded">{inst.systemSizeKw} kWp</span>
                     </div>
                     ...
                     <div className="pt-2 border-t border-slate-50 flex items-center justify-between">
                       ...
                       <div className="relative flex-1 mr-2" onClick={(e) => e.stopPropagation()}>
                          <select
                            ...
                            title={canEditStatus ? "Zmień status" : "Brak uprawnień do zmiany statusu"}
                          >
                       ...
                       <button className="text-xs text-blue-600 hover:text-blue-800 font-medium flex items-center whitespace-nowrap">
                         Szczegóły <ChevronRight className="w-3 h-3 ml-1" />
                       </button>
                     </div>
                   </div>
                 )})}
